...
import { InflationAnalysisCard } from './components/InflationAnalysisCard.jsx';
import { DividendCalendarCard } from './components/DividendCalendarCard.jsx';
import { IncomeReportCard } from './components/IncomeReportCard.jsx';
...
import { fetchTickersData } from './services/brapiService.js';
import { AVAILABLE_THEMES } from './services/themeService.js';

const AssetDetailModal = lazy(() => import('./components/AssetDetailModal.jsx').then(m => ({ default: m.AssetDetailModal })));
const DividendCalendarModal = lazy(() => import('./components/DividendCalendarModal.jsx').then(m => ({ default: m.DividendCalendarModal })));
const IncomeReportModal = lazy(() => import('./components/IncomeReportModal.jsx').then(m => ({ default: m.IncomeReportModal })));
const RealPowerModal = lazy(() => import('./components/RealPowerModal.jsx').then(m => ({ default: m.RealPowerModal })));
const EvolutionModal = lazy(() => import('./components/EvolutionModal.jsx').then(m => ({ default: m.EvolutionModal })));
const PortfolioModal = lazy(() => import('./components/PortfolioModal.jsx').then(m => ({ default: m.PortfolioModal })));
...
const App = () => {
    ...
    try {
        ...
        if (liveData && liveData.length > 0) {
          setAssets(prev => prev.map(asset => {
              const live = liveData.find(l => l.symbol === asset.ticker);
  ...
  const handleImportData = useCallback((newData) => {
  ...
  const handleSaveTransaction = useCallback((newTransaction) => {
