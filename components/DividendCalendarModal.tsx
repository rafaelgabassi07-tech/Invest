        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
            <div className="max-w-7xl mx-auto px-6 py-8 w-full h-full flex flex-col lg:flex-row gap-8">
                    ...
                    <div className="bg-white dark:bg-[#1c1c1e] rounded-[2.5rem] p-8 border border-gray-200 dark:border-white/5 shadow-xl relative overflow-hidden mb-6">
                        <div className="flex items-center justify-between mb-8">
                            ...
                            <div className="flex gap-2">
                                <button onClick={prevMonth} className="w-10 h-10 flex items-center justify-center rounded-full border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                                    <ChevronLeft size={20} />
                                </button>
                                <button onClick={nextMonth} className="w-10 h-10 flex items-center justify-center rounded-full border border-gray-200 dark:border-white/10 hover:bg-gray-100 dark:hover:bg-white/5 transition-colors">
                        ...
                        <div className="grid grid-cols-7 mb-4">
                            {WEEKDAYS.map(day => (
                                <div key={day} className="text-center text-xs font-bold text-gray-400 uppercase py-2 tracking-wider">
                                    {day.substring(0, 3)}
                                </div>
                            ))}
                        </div>
                                ...
                                return (
                                    <div key={day} className={`h-24 lg:h-32 rounded-2xl border p-2 flex flex-col justify-between transition-all hover:shadow-md ${isToday ? 'bg-indigo-50/50 dark:bg-indigo-500/10 border-indigo-500/30' : 'bg-gray-50/50 dark:bg-[#2c2c2e]/50 border-gray-100 dark:border-white/5 hover:border-gray-300 dark:hover:border-white/20'}`}>
                                        <div className="flex justify-between items-start">
                                            ...
                                            {hasEvent && (
                                                 <span className="text-[9px] font-bold text-emerald-600 dark:text-emerald-500 bg-emerald-500/10 px-1.5 py-0.5 rounded-md">
                                                     {dayEvents.length}
                                                 </span>
                                            )}
                                        </div>
                                        <div className="space-y-1 overflow-hidden">
                                            {dayEvents.slice(0, 3).map((ev, idx) => (
                                                <div key={idx} className="flex items-center gap-1.5 px-2 py-1 bg-white dark:bg-[#1c1c1e] rounded-lg shadow-sm border border-gray-100 dark:border-white/5">
                                                    <div className="w-1.5 h-1.5 rounded-full bg-indigo-500"></div>
                                                    <span className="text-[9px] font-bold text-gray-900 dark:text-white truncate">{ev.asset.ticker}</span>
                                                </div>
                                            ))}
                                            {dayEvents.length > 3 && (
                                                <div className="text-[9px] text-center text-gray-400 font-bold">+{dayEvents.length - 3} mais</div>
                                            )}
                                        </div>
                                    </div>
                                );
                ...
                <div className="w-full lg:w-96 flex flex-col gap-6">
                    <div className="bg-indigo-600 rounded-[2.5rem] p-8 text-white relative overflow-hidden shadow-2xl shadow-indigo-600/20 group">
                    ...
                    <div className="bg-white dark:bg-[#1c1c1e] rounded-[2.5rem] p-6 border border-gray-200 dark:border-white/5 shadow-xl flex-1 flex flex-col overflow-hidden">
                         ...
                         <div className="overflow-y-auto custom-scrollbar flex-1 pr-2 space-y-3">
                            {events.length > 0 ? events.map((ev, idx) => (
                                <div key={idx} className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 dark:bg-[#2c2c2e]/50 border border-gray-100 dark:border-white/5 hover:bg-white dark:hover:bg-[#2c2c2e] hover:shadow-md transition-all group">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 rounded-xl bg-white dark:bg-[#1c1c1e] flex flex-col items-center justify-center border border-gray-200 dark:border-white/5 shadow-sm">
                                            <span className="text-[10px] font-bold text-indigo-500 uppercase">{MONTHS[currentDate.getMonth()].substring(0,3)}</span>
                                            <span className="text-xl font-bold text-gray-900 dark:text-white leading-none">{ev.day}</span>
                                        </div>
                                        <div>
                                            <h4 className="font-bold text-gray-900 dark:text-white">{ev.asset.ticker}</h4>
                                            <p className="text-xs text-gray-500">{ev.asset.shortName}</p>
                                        </div>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-bold text-emerald-600 dark:text-emerald-500">+ R$ {ev.amount.toFixed(2)}</p>
